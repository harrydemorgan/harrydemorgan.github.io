import { Bed, Footprints, Heart, Activity, LucideIcon } from "lucide-react";
...
type MetricConfig = {
  id: string;
  key: keyof NonNullable<BiomarkersData['metrics']>;
  title: string;
  icon: LucideIcon;
...
function MetricCard({ config, value }: { config: MetricConfig; value: string | number }) {
  const Icon = config.icon;
  
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-zinc-200/80 bg-white shadow-sm transition-all duration-300 hover:shadow-xl hover:scale-[1.02] dark:border-zinc-800/80 dark:bg-zinc-900/50">
      <div className={`h-1 w-full ${config.colorClasses.line}`} />
      ...
      <div className="relative p-8">
        <div className={`mb-4 inline-flex items-center justify-center rounded-xl p-3 ${config.colorClasses.iconBg}`}>
          <Icon className={`h-6 w-6 ${config.colorClasses.iconColor}`} />
        </div>
...
export default function Home() {
  ...
  const availableMetrics = METRICS_CONFIG.filter(
    (config) => metrics && metrics[config.key] !== undefined && metrics[config.key] !== null
  );
